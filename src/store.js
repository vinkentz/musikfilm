...
export default new Vuex.Store({
  ...
  mutations: {
    ...
    DBPEDIA_LAGU(state, hasil){
      state.dbpedia_lagu = hasil
    },
    ...
    SHOW_LAGU(state, hasil){
      state.show_lagu = hasil
    },
    DETAIL_LAGU(state, hasil){
      state.detail_lagu = hasil
    },
    ...
    DETAILS_PENGARANG_LAGU(state, hasil){
      state.details_pengarang_lagu = hasil
    },
  },
  actions: {
      ...
      axios.get(urldbpedia, {
        params: {
          ...
          SELECT ?label ?comment ?link WHERE{
            ?subject rdfs:label "Lagu daerah"@id.
    ...
    showLagu({ commit }, id) {
      axios.get(link, {
        ...
        const result = response.data.results.bindings
        let hasil = result
        commit('SHOW_LAGU', hasil)
      })
    },

    detailLagu({ commit }, id) {
      axios.get(link, {
        params: {
          query: `
          PREFIX mf: <http://alunalun.info/ontology/musikfilm#>
          ...
          WHERE {
            ...
            OPTIONAL {?subject mf:fromProvinsi ?provinsi.
              ...
              OPTIONAL {?subject mf:hasPengarangLagu ?pengarang.
          ...
          axios.get(link, {
            ...
            nodeVisualisasi({ commit }) {
              axios.get(link, {
                ...
                detailsPengarangLagu({ commit }, id) {
                  axios.get(link, {
                    params: {
                      query: `
                      PREFIX rdfs: <http://www.w3.org/2000/01/rdf-schema#>
                      ...
                      SELECT ?subject ?label ?labellagu
                      WHERE{
                        {?object rdfs:label ?label.}
                        UNION{
                          ?subject mf:hasPengarangLagu ?object.
                          ?subject rdfs:label ?labellagu.
                        }
                       FILTER(<${id}> = ?object)
                      }`
                                          }
                    ...
                    const result = response.data.results.bindings
                    let hasil = result
                    commit('DETAILS_PENGARANG_LAGU', hasil)
                  })
                },
